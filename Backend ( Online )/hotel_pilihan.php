...
<body>
        ...
        <div class="container py-5">
            <div class="row justify-content-center py-5">
                <div class="col-lg-10 pt-lg-5 mt-lg-5 text-center">
                    ...
                    <div class="mt-5 pt-5">
                        <h2 class="text-white mb-4">Popular Destinations</h2>
                        <div id="featuredDestinations" class="row g-4 justify-content-center"></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    // Popular destinations to show by default
    const POPULAR_DESTINATIONS = ['Bali', 'Paris', 'Tokyo', 'New York', 'Santorini', 'Switzerland'];
    
    // Load featured destinations when page loads
    ...
    async function loadFeaturedDestinations() {
        ...
        try {
            ...
            // Fetch photos for each popular destination
            const destinationPromises = POPULAR_DESTINATIONS.map(dest => 
                fetchUnsplashPhotos(dest, 1).then(photos => ({
                    destination: dest,
                    photo: photos[0] // Get first photo for each destination
                }))
            );
            ...
            destinations.forEach((item, index) => {
                ...
                col.innerHTML = `
                    <div class="featured-card h-100">
                        <img src="${item.photo.url}" class="featured-img" alt="${item.photo.alt}">
                        <div class="featured-overlay">
                            <h3 class="featured-title">${item.destination}</h3>
                            <div class="d-flex justify-content-between align-items-center">
                                <span class="featured-location">
                                    <i class="fas fa-map-marker-alt me-2"></i> ${item.photo.location || 'Exotic Destination'}
                                </span>
                                <a href="${item.photo.photographerUrl}" target="_blank" class="photographer-badge text-white text-decoration-none">
                                    <i class="fas fa-camera me-1"></i> ${item.photo.photographer}
                                </a>
                            </div>
                        </div>
                    </div>`;
            ...
            container.innerHTML = `
                <div class="col-12 text-center py-3">
                    <div class="alert alert-warning">
                        Couldn't load featured destinations. Please try refreshing the page.
                    </div>
                </div>`;
        }
    }
    ...
    async function searchDestination() {
        ...
        try {
            const photos = await fetchUnsplashPhotos(query, 12);  // Changed count to 12
    ...
    async function fetchUnsplashPhotos(query, count = 12) {  // Changed count to 12
        const url = `https://api.unsplash.com/search/photos?query=${encodeURIComponent(query + ' vacation')}&client_id=${UNSPLASH_ACCESS_KEY}&per_page=${count}&orientation=landscape`;
            ...
            col.innerHTML = `
                <div class="destination-card h-100">
                    ...
                    <div class="p-4">
                        ...
                        <button class="btn btn-primary view-package-btn w-100" onclick="window.location.href='${photo.photographerUrl}'">
                            View Package Details <i class="fas fa-arrow-right ms-2"></i>
                        </button>
                        <button class="btn btn-link text-decoration-none w-100 mt-2" onclick="toggleReadMore(${index})">
                            Read More <i class="fas fa-arrow-down ms-2"></i>
                        </button>
                        <div id="more-info-${index}" class="mt-2 text-muted" style="display: none;">
                            <p>Here is more information about the ${query} vacation package. This includes everything from travel dates to included amenities and more!</p>
                        </div>
                    </div>
                </div>`;
            container.appendChild(col);
